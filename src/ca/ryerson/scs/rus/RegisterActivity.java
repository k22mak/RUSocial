	...
	public void onClick(View v) {
		...
		if (v == btnRegister) {
			if (SplashActivity.DEBUG) {
				...
				if (ValidityCheck.passwordCheck(
						evPassword.getText().toString(), evConfirmPassword
								.getText().toString()) == false) // !(ValidityCheck.passwordCheck("else",
																	// "no"))
				{
					...
					// set up button
					Button button = (Button) dialogUser
							.findViewById(R.id.Button01);
					button.setOnClickListener(new OnClickListener() {
						@Override
						public void onClick(View v) {
					...
					HttpRequestAdapter.httpRequest(this, URLResource.LOGIN,json,new RegisterHandler());
